function [niceBarplot_figure] = lw_niceBARplot(meanvec,semvec,label_title,label_yaxis,label_xticks,limit_yaxis,varargin)
    ...
    ba = bar(p.Results.meanvec,'grouped','EdgeColor','none','BarWidth',0.85, 'FaceColor', 'flat');
    % insert errorbars
    hold on
        ...
        % Plot the errorbars
        errorbar(x',p.Results.meanvec,p.Results.semvec,'k','linestyle','none');
    hold off
    barcolcount = 1;
    for barcol = 1:nbars
        set(ba(barcol),'facecolor',rgbcol(barcol,:),'LineStyle','-')
    end % barcol
